...
| Xtream Codes | Capability-oriented live, EPG, stream-resolution, VOD, and series provider adapter with secure credential retrieval | Encoded `player_api.php`, authentication response validation, live/short-EPG/VOD/series DTO retrieval, factory registration, and validated live playback URL construction | Canonical `Channel`, `EPGEntry`, `Movie`, and `Series` translation with local channel search | Focused request-builder, API-client, DTO translation, adapter, and registration tests | Direct live URL resolution; no playback engine | **Partially implemented** |
