...
| MAG/Stalker compatibility lab | Protocol profile and middleware-family investigation increment | Legacy, Stalker-query, GUI, and helper request profiles; bounded six-candidate discovery; deterministic response/error and resource-lifecycle fixtures; a source-derived local classic middleware lab covering handshake, token transport, genres, page-one/page-two ordered lists, channel translation, and `create_link`; controlled session re-authentication tests; redacted protocol diagnostics; and documented official/open-source middleware evidence boundaries. Production portal compatibility remains unresolved because no authorized candidate returned a token-bearing handshake. |
