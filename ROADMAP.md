...
| Xtream Codes | Capability-oriented live, EPG, VOD, and series provider adapter with secure credential retrieval | Encoded `player_api.php`, authentication response validation, live/short-EPG/VOD/series DTO retrieval, and factory registration | Canonical `Channel`, `EPGEntry`, `Movie`, and `Series` translation with local channel search | Focused request-builder, API-client, DTO translation, adapter, and registration tests | None | **Partially implemented** |
