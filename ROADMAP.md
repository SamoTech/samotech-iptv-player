...
| Xtream Codes | Capability-oriented live and VOD provider adapter with secure credential retrieval | Encoded `player_api.php`, authentication response validation, live/VOD DTO retrieval, and factory registration | Canonical `Channel` and `Movie` translation with local channel search | Focused request-builder, API-client, DTO translation, adapter, and registration tests | None | **Partially implemented** |
