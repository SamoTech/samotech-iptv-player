...
| Xtream Codes | Capability-oriented live provider adapter with secure credential retrieval | Encoded `player_api.php`, authentication response validation, live-stream DTO retrieval, and factory registration | Canonical `Channel` translation with local channel search | Focused request-builder, API-client, DTO translation, adapter, and registration tests | None | **Partially implemented** |
