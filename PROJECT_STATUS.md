...
| Xtream Codes API | Provider/content source | **Partially Implemented** | Credential validation; live channels; live/VOD/series categories; movies; Movie detail metadata; Series, Season, and Episode discovery; short EPG; local Live search; and typed live/Movie/Episode stream resolution. The desktop shell exposes Movie detail/play and Series → Season → Episode navigation. | Request builder, API client, DTO translator, adapter, resolver, Movie-detail, season/episode, unified target playback, registration, desktop-composition, and native Qt flow coverage. | Registered Live, Movie, and Episode targets all resolve through the same player path; Series is a browsable container rather than a playable target. | Validate the complete VOD and Series flow against an authorized real Xtream provider; server-side non-live search, richer metadata/presenter styling, and broader provider support remain future work. |
