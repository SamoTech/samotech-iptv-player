...
| Xtream Codes API | Provider/content source | **Partially Implemented** | Credential validation; live channels; live/VOD/series category adapter methods; movies; series; short EPG; local live-channel search; live stream URL construction/resolution; and registered **live-category** browsing. | Request builder, API client, DTO translator, adapter, resolver, registration, live-category integration, and EPG coverage. | Live stream resolution is available to the registered-player use case; live-category discovery is browse-only. | Add VOD/series/episode resolver, use-case, and UI workflows; no VOD or series playback claim. |
...
| Movies/VOD | Content type | **Partially Implemented** | Domain `Movie`; Xtream adapter `load_movies()`; browse-only Movie catalogue; provider-neutral Movie-playback resolver contract. | Domain, browse, resolver, target, and fake contract tests. | No provider advertises Movie playback and no registered movie-playback route or UI exists. | Concrete authorized adapter implementation, target dispatch, and explicit playback UI. |
| Series | Content type | **Partially Implemented** | Domain `Series`, provider-scoped `Season`, browse-only Series catalogue, and provider-neutral Series-detail discovery contracts. | Domain, fake discovery/race, resolver, and browse tests. | Series remains non-playable; no provider advertises Series detail. | Concrete authorized seasons/episodes adapter implementation and UI navigation. |
| Episodes | Content type | **Partially Implemented** | Canonical Episode record plus a provider-scoped safe Episode DTO/request boundary, generation-safe discovery use case, and provider-neutral Episode-playback resolver contract. | Domain, fake discovery/race, resolver, target, and raw-URL-rejection tests. | No provider advertises Episode discovery or playback. | Concrete authorized adapter implementation, target dispatch, and explicit playback UI. |
...
| Technology | Category | Current status | Implemented | Tested | Playback | Remaining work |
|---|---|---:|---|---|---|---|
| PySide6/Qt desktop shell | Desktop UI | **Partially Implemented** | Main window, native video surface, provider-entry dialogs, provider list, channel browser, browse-only live-category dialog, provider-native EPG dialog, local XMLTV configuration/manual-refresh dialog, generic pause/resume/stop and recording actions, settings action, production composition, and supported source-install entry points. | Fake-backed presentation, bootstrap, composition-root, lifecycle, entry-point, category-dialog, and XMLTV-dialog tests. | One shared libVLC player is injected into registered-provider playback, playback controls, recording, and the native video surface; category and XMLTV guide browsing have no player path. | Category-to-channel navigation, active-item/state detail, and release packaging. |
...
3. Registered Xtream live-category discovery and capability-driven Movie/Series catalogue browsing are browse-only workflows. Provider-neutral Movie/Episode resolution and Series-detail discovery contracts now exist, but no concrete M3U, MAG/Stalker, or Xtream adapter advertises or executes them; no non-live stream-resolution path or playback claim exists.
