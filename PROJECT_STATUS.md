...
**Current baseline:** The `main` revision containing deterministic bundled-VLC selection for source and frozen execution, durable startup checkpoints, and the prior bounded Live EOF recovery and Windows validation increments; use `git rev-parse HEAD` for its immutable revision identifier.
...
The release process is **IMPLEMENTED and VERIFIED for v0.1.0 and v0.1.1**: non-tag artifacts use the short-SHA naming form, tag runs use the versioned naming form, dynamic release notes consume current build metadata and project-status limitations, `SHA256SUMS.txt` is uploaded beside the executable, and release publication remains after all blocking gates. Windows presentation test modules remain **BLOCKED as a runner-environment limitation** because collection caused a fatal Qt access violation; they are excluded explicitly while the non-presentation Windows corpus remains blocking. Installer creation, ARM64, auto-update infrastructure, authorized-provider acceptance, and production subtitle interoperability remain **DEFERRED**, **OUT OF SCOPE**, or **NOT EXECUTED** as applicable.
...
| qasync runtime | Desktop lifecycle | **Implemented** | Qt-aware asyncio event loop, lifecycle entry point, generic startup failures, failure-only atomic startup journal, ordered checkpoints through `MAIN_WINDOW_SHOWN`, and shared HTTP cleanup after the window loop exits. | Focused runtime, entry-point, diagnostics, and Windows workflow-contract tests. | Supports asynchronous UI orchestration. | Broader crash-reporting policy and end-user collection UX. |
...
1. A production composition root and source-install lifecycle entry point now initialize safe SQLite state, restore provider metadata, wire registered-provider use cases, load the persisted theme, run qasync, report generic startup failures, close the shared HTTP resource, and retain a redacted failure-only startup journal with ordered checkpoints. Packaging, installers, update delivery, crash-reporting policy, and user-facing diagnostic collection remain incomplete.
