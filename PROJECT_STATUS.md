...
| Technology | Category | Current status | Implemented | Tested | Playback | Remaining work |
|---|---|---:|---|---|---|---|
| PySide6/Qt desktop shell | Desktop UI | **Partially Implemented** | Main window, native video surface, provider-entry dialogs, provider list, channel browser, browse-only live-category dialog, EPG dialog, generic pause/resume/stop and recording actions, settings action, production composition, and supported source-install entry points. | Fake-backed presentation, bootstrap, composition-root, lifecycle, entry-point, and category-dialog tests. | One shared libVLC player is injected into registered-provider playback, playback controls, recording, and the native video surface; category browsing has no player path. | Category-to-channel navigation, active-item/state detail, and release packaging. |
...
**Next bounded task:** Bind XMLTV sources to registered-provider configuration only after defining safe source metadata, fetch, mapping persistence, refresh, and error boundaries. This remains separate from VOD, series, episode, catch-up, and playback work.
