...
class TestProviderRegistry:
    def test_register_and_get(self, registry: ProviderRegistry,
                              mag_meta: InfraProviderMetadata) -> None:
        registry.register(mag_meta)
        retrieved = registry.get("mag1")
        assert retrieved.provider_id == "mag1"

    def test_get_raises_not_found_for_unknown(self,
    ...
    def test_deregister_returns_true(self, registry: ProviderRegistry,
                                     mag_meta: InfraProviderMetadata) -> None:
        registry.register(mag_meta)
        assert registry.deregister("mag1") is True
        assert "mag1" not in registry

    def test_deregister_returns_false_when_not_present(
        self, registry: ProviderRegistry
    ) -> None:
        assert registry.deregister("ghost") is False

    def test_list_all(self, registry: ProviderRegistry,
                      mag_meta: InfraProviderMetadata) -> None:
        registry.register(mag_meta)
        registry.register(InfraProviderMetadata(
            provider_id="m3u1", provider_type="m3u", base_url="http://m3u.example.com"
        ))
        assert len(registry.list_all()) == 2

    def test_list_by_type(self, registry: ProviderRegistry,
                          mag_meta: InfraProviderMetadata) -> None:
        registry.register(mag_meta)
        registry.register(InfraProviderMetadata(
            provider_id="m3u1", provider_type="m3u", base_url="http://m3u.example.com"
    ...
    def test_list_active_filters_inactive(self,
                                          registry: ProviderRegistry) -> None:
        registry.register(InfraProviderMetadata(
            provider_id="active", provider_type="mag",
            base_url="http://a.com", is_active=True,
        ))
        registry.register(InfraProviderMetadata(
            provider_id="inactive", provider_type="mag",
            base_url="http://b.com", is_active=False,
        ))
        assert len(registry.list_active()) == 1
        assert registry.list_active()[0].provider_id == "active"

    def test_contains_operator(self, registry: ProviderRegistry,
                               mag_meta: InfraProviderMetadata) -> None:
        registry.register(mag_meta)
        assert "mag1" in registry
        assert "ghost" not in registry

    def test_len(self, registry: ProviderRegistry,
                 mag_meta: InfraProviderMetadata) -> None:
...
class TestProviderFactory:
    def test_register_type_and_create(self,
                                      mag_meta: InfraProviderMetadata) -> None:
