...
For a normal push, the artifact is named `SamoTech-IPTV-Player-Windows-x64-build-<shortsha>.exe`. A version tag such as `v0.1.1` produces `SamoTech-IPTV-Player-Windows-x64-v0.1.1.exe`; the workflow automatically generates the release body from the current version, build metadata, checksum, validation summary, and authoritative project-status limitations, then publishes the executable and `SHA256SUMS.txt` after all blocking gates pass. No manual release-note editing or EXE upload is required. The current verified build evidence is recorded in [ZERO_TOUCH_WINDOWS_RELEASE_AUDIT.md](ZERO_TOUCH_WINDOWS_RELEASE_AUDIT.md).

The generated executable is tested from outside the repository with a sanitized Windows `PATH`, including a temporary directory containing spaces and non-ASCII characters. The packaged runtime discovers VLC relative to the executable rather than through the working directory or a system VLC installation. The current release scope is Windows x64 only; installers, auto-updaters, ARM64 builds, and remote/tokenized XMLTV sources remain out of scope or deferred. The tag-to-release path is intended to be zero-touch: create the semantic version tag, push it, and let the blocking workflow build, validate, checksum, upload, and publish the release.
