...
| MAG/Stalker | **Partially Implemented** | Authorized MAC identity handling, bounded fixed-candidate handshake discovery, failure-safe owned HTTP-session cleanup, explicit session lifecycle, legacy and Stalker-query request profiles, live channels, EPG, local channel search, and live stream resolution. | Production portal compatibility remains provider-specific: a candidate must return a structurally valid token-bearing handshake before channels or playback can be claimed. Windows/libVLC availability is separately observed, but MAG remains blocked before stream resolution. Canonical VOD, series, category-family, catch-up/archive, and user-facing catalogue workflows remain open. |
