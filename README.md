...
| MAG/Stalker | **Partially Implemented** | Authorized MAC identity handling, bounded fixed-candidate handshake discovery, explicit session lifecycle, legacy and Stalker-query request profiles, live channels, EPG, local channel search, and live stream resolution. | Production portal compatibility remains provider-specific: a candidate must return a structurally valid token-bearing handshake before channels or playback can be claimed. Canonical VOD, series, category-family, catch-up/archive, and user-facing catalogue workflows remain open. |
