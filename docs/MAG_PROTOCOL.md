...
The supplied real portal remains **UNRESOLVED** for compatibility. After the new source-backed state-machine implementation, the six fixed differential tests T01–T06 were run once against the authorized portal: GUI GET/POST and GUI POST-prehash returned HTTP 404 `text/javascript` zero-byte responses; helper GET prehash=false/0 and helper POST prehash=0 returned HTTP 404 `text/html` 146-byte responses. One explicit MAG254 helper request returned HTTP 404 `text/html` 146 bytes. No test returned JSON, a token, a profile, an error field, or an authorization policy marker. The most recent bounded authorized validation completed the six-candidate closed set:
