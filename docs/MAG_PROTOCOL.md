...
The supplied real portal remains **UNRESOLVED** for compatibility. After the committed URL-preserving and source-observed-key changes, the authorized T01–T06 matrix was rerun. GUI GET/POST and GUI POST-prehash returned HTTP 404 `text/javascript` zero-byte responses; helper GET prehash=false/0 and helper POST prehash=0 returned HTTP 404 `text/html` 146-byte responses. Every response reported `Server: nginx`, zero redirects, no `Allow` header, and no `WWW-Authenticate` header. No test returned JSON, a token, a profile, an error field, or an authorization policy marker. One earlier explicit MAG254 helper request also returned HTTP 404 `text/html` 146 bytes. The most recent bounded authorized validation completed the six-candidate closed set:
