...
The adapter also runs a cancellable media-level heartbeat for typed LIVE resources after native `PLAYING`. It samples libVLC media time through `get_time()` at a bounded interval, records progress only when the position advances, and routes an unchanged position beyond the configured `live_stall_timeout_s` (15 seconds by default) through the existing bounded recovery task as `STALLED`. The heartbeat is generation/session scoped, is cancelled on recovery, stop, pause, channel switch, recording restart, and shutdown, and does not poll `is_playing()` as a health proxy. Legacy URL-only callers retain the existing live-recovery behavior but do not receive typed liveness classification.

Native `END_REACHED` is classified using the existing `PlaybackResource`. LIVE resources remain eligible for bounded `EOF` recovery. MOVIE and EPISODE resources transition to the existing public `ENDED` state without rebuilding media. Typed non-live STOPPED and ENCOUNTERED_ERROR events do not enter the live recovery path. Intentional actions and stale generations remain ignored.
...
| Live media heartbeat / stall detection | Implemented for typed LIVE resources through bounded `get_time()` position sampling and existing `STALLED` recovery |
| VOD/episode END classification | Implemented using `PlaybackResource`; typed MOVIE/EPISODE completion reaches `ENDED` without live recovery |
...
The subsequent media-reliability remediation adds a typed LIVE heartbeat to the same adapter. Native `PLAYING` starts generation-scoped position sampling; media-time advancement emits safe `MEDIA_PROGRESS` diagnostics, while an unchanged position beyond the configured threshold routes `STALLED` through the existing recovery task. The same resource context differentiates live interruption recovery from normal MOVIE/EPISODE completion and prevents typed non-live failures from entering live recovery.
...
The reliability policy remains deliberately narrow. Ordinary `BUFFERING` waits for the existing watchdog threshold; LIVE `END`, unexpected LIVE `STOPPED`, LIVE `ENCOUNTERED_ERROR`, and typed LIVE `STALLED` may request one bounded live-media rebuild when the generation/session is current. Typed MOVIE/EPISODE `END` reaches normal completion, while typed non-live STOPPED/ERROR do not reuse live recovery. Intentional stop, shutdown, stale callbacks, duplicate pending recovery, attempt exhaustion, and recovery-window exhaustion do not restart media. Live/VOD cache defaults remain unchanged at the configured 1000 ms because the player boundary still lacks evidence-backed separate cache profiles.

The implementation is **PROVEN** by deterministic synthetic provider/playback tests and the successful Windows hosted run `32159903781` for commit `04a9d1b`. The media-stall remediation is **FIXED at the typed libVLC boundary**: a stalled LIVE position triggers the existing bounded recovery policy, and typed VOD/episode END reaches normal completion. It remains **PARTIAL** for provider-specific media headers, temporary URL refresh, and portal lifetime behavior. M3U User-Agent/Referer propagation is **IMPLEMENTED**; Xtream and MAG media-plane metadata remain **REQUIRES AUTHORIZED PROVIDER VALIDATION**; MAG watchdog behavior remains **REQUIRES AUTHORIZED PROVIDER VALIDATION**; populated real-provider continuity remains **NOT TESTED**.
