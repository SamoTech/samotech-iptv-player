...
| Control plane versus media plane | Provider adapters resolve catalogue/session/link data; libVLC receives the final HTTP(S) target and media options. | Portal/API/session/watchdog/create-link control is separate from Enigma2 service playback. | Xtream API/session/catch-up control is separate from Enigma2 service playback. | IMPLEMENTED / EXPLICIT | Current SamoTech adapters do not claim provider-specific media cookies/watchdog propagation without evidence. |
| Enigma2 service types `1/4097/5001/5002/8193` | Not used; these are not VLC protocols. | Values select Enigma2 service/player backends and may vary by installed binaries. | Values select Enigma2 service/player backends and may vary by installed binaries. | REJECTED FOR DIRECT PORT | SamoTech uses `ResolvedPlayback` → `PlayerPort` → libVLC MediaPlayer. |
...
- [KIDDAC technology gap matrix](../historical/KIDDAC_TECHNOLOGY_GAP_MATRIX.md)
