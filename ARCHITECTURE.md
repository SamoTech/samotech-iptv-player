...
Current provider adapters are capability-oriented. A provider implements only the applicable contracts such as `CatalogProvider`, `PlaybackProvider`, `EPGProvider`, `SearchProvider`, `VodProvider`, `SeriesProvider`, `MoviePlaybackProvider`, `SeriesDetailProvider`, `EpisodePlaybackProvider`, `CategoryProvider`, `AuthenticationProvider`, or `SessionProvider`. The adapter’s actual advertised capabilities are the support claim—not a generic interface, an enum value, or a future design note. The new non-live contracts are intentionally not advertised by M3U, MAG/Stalker, or Xtream until a concrete adapter implements them against authorized sanitized fixtures.

## Stream resolution and playback

The application’s `PlayChannel` use case resolves an authorized canonical URL through `PlaybackProvider` and passes it to `PlayerPort`. `PlayRegisteredChannel` first resolves the selected provider from the provider resolver, then delegates the same provider-to-player boundary. The provider-neutral `MoviePlaybackProvider` and `EpisodePlaybackProvider` contracts reserve the same provider-to-player resolution seam for a later dispatch increment, while `SeriesDetailProvider` returns only canonical Season/Episode records. No current adapter implements these non-live contracts, and the Qt presentation layer neither resolves provider streams nor accesses credentials.
