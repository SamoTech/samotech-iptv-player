...
M3U now resolves current parsed HTTP(S) streams through `PlaybackProvider`, advertises `STREAM_RESOLUTION`, and reaches libVLC through the registered-provider path. The primary current gap is generic playback-state controls, not another player abstraction: the next increment should surface pause, resume, and stop through the existing libVLC-only `PlayerPort` without exposing stream URLs. Other gaps—including VOD/series workflows, XMLTV source binding, provider management, and production hardening—are tracked in [PRODUCT_GAP_ANALYSIS.md](PRODUCT_GAP_ANALYSIS.md).
